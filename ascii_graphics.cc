...
card_template_t combine_templates(card_template_t ctt1, card_template_t ctt2) {
	if (ctt1.size() ==  ctt2.size()) {
		card_template_t out;
		for (size_t it = 0; it < ctt1.size(); ++it) {
			out.push_back(ctt1[it] + ctt2[it]);
		}
		return out;
  	}
	std::cerr << "combining cards unsuccessful, two cards had different lengths" << std::endl;
	return ctt1;
}
...
card_template_t display_minion_no_ability(std::string name,int cost,int attack,int defence, bool action) {
  return display_minion_general(CARD_TEMPLATE_MINION_NO_ABILITY,name,cost,attack,defence,"",0, action);
}

card_template_t display_minion_triggered_ability(std::string name,int cost,int attack,
                                                 int defence,std::string trigger_desc, bool action) {
  return display_minion_general(CARD_TEMPLATE_MINION_NO_ABILITY,name,cost,attack,
                                defence,trigger_desc,0, action);
}

card_template_t display_minion_activated_ability(std::string name,int cost,int attack, int defence,
                                                 int ability_cost, std::string ability_desc, bool action) {
  return display_minion_general(CARD_TEMPLATE_MINION_WITH_ABILITY,name,cost,attack,defence,
                                ability_desc,ability_cost, action);
}
...
static card_template_t display_minion_general(card_template_t out,std::string name,int cost,int attack,int defence,std::string desc,int ability_cost, bool action) {
  std::ostringstream oss;
  prepare_for_replace(out);
  replace_text_left(out,'N',name);
  oss << cost;
  ...
  replace_text_left(out,'E',desc);
  oss.str("");
  if (action) oss.str("Action");
  else oss.str("");
  replace_text_left(out, 'Z', oss.str());
  oss.str("");
  if (ability_cost < 0) oss << 'X';
  else oss << ability_cost;
  replace_text_left(out,'K',oss.str());
  return out;
}
